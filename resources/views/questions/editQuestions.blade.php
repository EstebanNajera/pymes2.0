	    ...
	    <form method="POST" action="{{ route('questions.update', $question -> id_question) }}">
			...
			<div class="card border-info">
				...
				<div class="card-body col-md-10 offset-md-1">
					...
					<div class="row form-group" id="closedAnswersL" style="{{ $question -> type === "MultipleC" ? "display: visible;" : "display: none;" }}">
						...
						@else
						    @for($i = 0; $i < $options -> count(); $i++)
						        <div class="row form-group col-md-12" id="{{ "r_".$i }}">
							        <div class="col-md-8">
					...
					<div class="row text-center form-group">
					    <label class="checkbox-inline"><input type="checkbox" name="status" value="1" {{ $question -> status === 1 ? "checked" : "" }}>Activa</label>
					</div>

					<div class="row form-group">
		                <span class="input-group-btn col-md-2 offset-md-5">
		    	            <input type="submit" class="btn btn-info btn-lg" value="Guardar" size="10"></input>
		                </span>
					</div>
				</div>
			</div>
